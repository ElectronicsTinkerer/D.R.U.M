...
void TwoWire::setClock(uint32_t rate)
{
    ...
    if (_i2c_channel == 0) {
        i2c_set_baudrate(i2c0);
    }
    else {
        i2c_set_baudrate(i2c1);
    }
}
...
{
    if (transmitting) {
        if (_i2c_tx_buf_len >= I2C BUFFER_LENGTH) {
            return 0;
        }
...
uintu_t TwoWire::requestFrom(uint8_t addr, size_t len, uint8_t stop)
{
